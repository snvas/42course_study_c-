...
template <class K, class T> struct rbNode
{
	K key;
	T val;
	Color color;
...
// destructor
template <class K, class T> rbTree<K, T>::~rbTree()
{
	if (root != nullptr)
		delete root;
}

// inser pair to appropriate position, then update the tree to keep its
// properties
template <class K, class T> void rbTree<K, T>::insert(K &key, T &val)
{
	...
	// navigate to desire position of new node
	...
	if (cmp(key, curr->key) == -1)
		...
		curr->right = node;
	// update the tree recursively to kepp its preperties if needed
...
// remove the node with the 'key' from tree, then
// update the tree to keep its properties if needed
// this->size will decrease by 1
template <class K, class T> bool rbTree<K, T>::remove(const K &key)
{
...
// remove giben node from this tree, 'node' must be in this tree
template <class K, class T> void rbTree<K, T>::removeNode(rbNode<K, T> *node)
{
	if (node->color == RED)
	{
	...
	else
	{
		...
		{
			// no child. As the clack node deleted is a leaf, fixup
			// is necessary after delete
			if (node->parent == nullptr)
			{
				// node is root
				free(node);
			...
			{
				// you can guess, node->parent->parent must be
				// RED
...
// retunr 0 if a = b
// retunr 1 if a > b
template <class K, class T> int rbTree<K, T>::cmp(const K &a, const K &b) const
{
...
template <class K, class T> void rbTree<K, T>::leftRotate(rbNode<K, T> *node)
{
...
template <class K, class T> void rbTree<K, T>::rightRotate(rbNode<K, T> *node)
{
...
template <class K, class T> void rbTree<K, T>::printTree() const
{
...
template <class K, class T> int rbTree<K, T>::getSize() const
{
	return this->size;
}

// delete all nodes in this tree, recicle the memory
template <class K, class T> void rbTree<K, T>::clear()
{
	delete this->root;
	this->root = nullptr;
}
